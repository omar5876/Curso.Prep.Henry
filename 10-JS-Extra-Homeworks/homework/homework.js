...
function capToFront(s) {
  ...
  //Escribe tu código aquí
  let palabra = [...s].sort((a, b) => /[A-Z]/.test(a) ? /[A-Z]/.test(b) ? 0 : -1 : 0).join("");
  return palabra;
}
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  let palabra = str.split(" ").map(e => e.split("").reverse().join("")).join(" ");
  return palabra;
} 
...
function capicua(numero){
  ...
  if(number === reverso) return "Es capicua";
...
function buscoInterseccion(arreglo1, arreglo2){
  ...
  //Escribe tu código aquí
  const interseccion = arreglo1.filter(e => arreglo2.includes(e)); 
  return interseccion;
}
